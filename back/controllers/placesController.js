...
class PlacesController {
  ...
  static async deletePlace(req, res) {
    ...
    try {
      ...
      if (place.image) {
        ...
        if (fs.existsSync(filePath)) {
          fs.unlinkSync(filePath);
          console.log("🗑️ Imagem deletada junto com o local:", filename);
        }
      }
  ...
  // 🧩 NOVO: REDUZIR CAPACIDADE AO FAZER RESERVA
  static async reduceCapacity(placeId) {
    try {
      ...
      console.log(
        `📉 Capacidade do local ${place.name} atualizada: ${place.capacity}`
      );
      return place.capacity;
    } catch (error) {
      console.error("❌ Erro ao reduzir capacidade:", error);
      throw error;
    }
  }
}
